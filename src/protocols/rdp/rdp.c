...
#if defined(HAVE_FREERDP_EVENT_PUBSUB) && !defined(FREERDP_2)
/**
...
BOOL rdp_freerdp_pre_connect(freerdp *instance)
{
	...
	freerdp_register_addin_provider(guac_freerdp_channels_load_static_addin_entry, 0);
#endif
	...
	if (!(settings->disable_copy && settings->disable_paste) && freerdp_channels_load_plugin(channels, instance->settings, "cliprdr", NULL))
	{
		guac_client_log(client, GUAC_LOG_WARNING,
						"Failed to load cliprdr plugin. Clipboard will not work.");
	}
	/* If RDPSND/RDPDR required, load them */
	if (settings->printing_enabled || settings->drive_enabled || settings->audio_enabled)
	{
		/* Load RDPDR plugin */
	...
	{
		guac_client_log(client, GUAC_LOG_WARNING, "Failed to init gdi");
		return FALSE;
	}
#endif
	/* Init FreeRDP cache */
...
#ifdef FREERDP_2
	glyph = context->graphics->Glyph_Prototype;
#else
	free(bitmap);
...
#ifdef FREERDP_2
static void guac_rdp_post_disconnection(freerdp *rdp)
{
	rdpContext *context = rdp->context;
	guac_client *client = ((rdp_freerdp_context *)context)->client;
	guac_client_log(client, GUAC_LOG_DEBUG, "RDP client disconnected");
	PubSub_UnsubscribeChannelConnected(context->pubSub, (pChannelConnectedEventHandler)guac_rdp_channel_connected);
	PubSub_UnsubscribeChannelDisconnected(context->pubSub, (pChannelDisconnectedEventHandler)guac_rdp_channel_disconnected);
}
#endif
/**
 ...
 */
static int rdp_guac_client_wait_for_messages(guac_client *client, int timeout_msecs)
{
	guac_rdp_client *rdp_client = (guac_rdp_client *)client->data;
	...
	HANDLE handles[64];
	DWORD nCount = 0;
	DWORD tmp = freerdp_get_event_handles(rdp_inst->context, &handles[nCount], ARRAYSIZE(handles) - nCount);
	if (tmp < 0)
	{
		guac_client_abort(client, GUAC_PROTOCOL_STATUS_SERVER_ERROR, "Unable to read RDP event handles.");
		return -1;
	}
	nCount += tmp;
	DWORD waitStatus = WaitForMultipleObjects(nCount, handles, FALSE, timeout_msecs);
	...
	else if (waitStatus == WAIT_OBJECT_0)
	{
		...
		PubSub_OnTimer(rdp_inst->context->pubSub, rdp_inst->context, &timerEvent);
	}
	return tmp;
#else
	...
	if (result < 0)
	{
		...
		guac_client_abort(client, GUAC_PROTOCOL_STATUS_UPSTREAM_UNAVAILABLE, "Error waiting for file descriptor.");
		return -1;
	}
...
{
	guac_client_log(client, GUAC_LOG_DEBUG, "start guac_rdp_handle_connection");
	guac_rdp_client *rdp_client = (guac_rdp_client *)client->data;
	guac_rdp_settings *settings = rdp_client->settings;
	...
	guac_client_log(client, GUAC_LOG_DEBUG, "init guac_common_display_alloc");
	/* Create display */
	rdp_client->display = guac_common_display_alloc(client,
													rdp_client->settings->width,
													rdp_client->settings->height);
	guac_client_log(client, GUAC_LOG_DEBUG, "init current_surface");
	rdp_client->current_surface = rdp_client->display->default_surface;
#ifndef FREERDP_2
	rdp_client->requested_clipboard_format = CB_FORMAT_TEXT;
#endif

	guac_client_log(client, GUAC_LOG_DEBUG, "init guac_common_list_alloc");
	...
	guac_client_log(client, GUAC_LOG_DEBUG, "init freerdp_new");
	/* Init client */
...
#ifdef FREERDP_2
	rdp_inst->PostDisconnect = guac_rdp_post_disconnection;
#endif
	...
	guac_client_log(client, GUAC_LOG_DEBUG, "init freerdp_context_new");
	freerdp_context_new(rdp_inst);
	((rdp_freerdp_context *)rdp_inst->context)->client = client;

	/* Load keymap into client */
	rdp_client->keyboard = guac_rdp_keyboard_alloc(client, settings->server_layout);
	guac_client_log(client, GUAC_LOG_DEBUG, "init guac_common_cursor_set_pointer");

	/* Set default pointer */
	guac_common_cursor_set_pointer(rdp_client->display->cursor);

	guac_client_log(client, GUAC_LOG_DEBUG, "init guac_rdp_push_settings");
	/* Push desired settings to FreeRDP */
	guac_rdp_push_settings(client, settings, rdp_inst);

	guac_client_log(client, GUAC_LOG_DEBUG, "init freerdp_connect");
	...
	while (client->state == GUAC_CLIENT_RUNNING && !guac_rdp_disp_reconnect_needed(rdp_client->disp))
	{
		...
		guac_client_log(client, GUAC_LOG_DEBUG, "guac_rdp_handle_connection: %d.", wait_result);
		if (wait_result > 0)
		{
			...
			do
			{
				...
				{
					if (rdp_inst->settings->AutoReconnectionEnabled && freerdp_reconnect(rdp_inst))
					{
						pthread_mutex_unlock(&(rdp_client->rdp_lock));
						continue;
					}
					if (freerdp_get_last_error(rdp_inst->context) == FREERDP_ERROR_SUCCESS)
						guac_client_abort(client, GUAC_PROTOCOL_STATUS_UPSTREAM_NOT_FOUND, "Failed to check FreeRDP file descriptor");
					pthread_mutex_unlock(&(rdp_client->rdp_lock));
					wait_result = -1;
					break;
				}
#else
				if (!freerdp_check_fds(rdp_inst) && !freerdp_channels_check_fds(channels, rdp_inst)))
					{
				...
				if (required_wait > GUAC_RDP_FRAME_TIMEOUT)
					wait_result = rdp_guac_client_wait_for_messages(client, required_wait);
		...
		/* If a low-level connection error occurred, fail */
			...
			guac_client_abort(client, GUAC_PROTOCOL_STATUS_UPSTREAM_UNAVAILABLE, "Connection closed.");
	...
	guac_client_log(client, GUAC_LOG_INFO, "RDP client disconnected[%d]:%d", client->state, rdp_client->disp->reconnect_needed);
...
void *guac_rdp_client_thread(void *data)
{
	...
	if (settings->drive_enabled)
	{
		...
		rdp_client->filesystem = guac_rdp_fs_alloc(client, settings->drive_path,
