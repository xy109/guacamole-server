...
guac_transfer_function guac_rdp_rop3_transfer_function(guac_client *client, int rop3)
{
    ...
    /* "DSon" !(src | dest) */
    case 0x11:
        return GUAC_TRANSFER_BINARY_NOR;

    /* "DSna" !src & dest */
    case 0x22:
        return GUAC_TRANSFER_BINARY_NSRC_AND;

    /* "Sn" !src */
    case 0x33:
        return GUAC_TRANSFER_BINARY_NSRC;

    /* "SDna" (src & !dest) */
    case 0x44:
        return GUAC_TRANSFER_BINARY_NDEST_AND;

    /* "Dn" !dest */
    case 0x55:
        return GUAC_TRANSFER_BINARY_NDEST;

    /* "SRCINVERT" (src ^ dest) */
    case 0x66:
        return GUAC_TRANSFER_BINARY_XOR;

    /* "DSan" !(src & dest) */
    case 0x77:
        return GUAC_TRANSFER_BINARY_NAND;

    /* "SRCAND" (src & dest) */
    case 0x88:
        return GUAC_TRANSFER_BINARY_AND;

    /* "DSxn" !(src ^ dest) */
    case 0x99:
        return GUAC_TRANSFER_BINARY_XNOR;

    /* "MERGEPAINT" (!src | dest)*/
    case 0xBB:
        return GUAC_TRANSFER_BINARY_NSRC_OR;

    /* "SDno" (src | !dest) */
    case 0xDD:
        return GUAC_TRANSFER_BINARY_NDEST_OR;

    /* "SRCPAINT" (src | dest) */
    case 0xEE:
        return GUAC_TRANSFER_BINARY_OR;
...
BOOL guac_rdp_gdi_dstblt(rdpContext *context, const DSTBLT_ORDER *dstblt)
{
    ...
    case 0:
        ...
        guac_common_surface_set(current_surface, x, y, w, h, 0x00, 0x00, 0x00, 0xFF);
        break;

    /* DSTINVERT */
    case 0x55:
        guac_common_surface_transfer(current_surface, x, y, w, h, GUAC_TRANSFER_BINARY_NDEST, current_surface, x, y);
        break;
    ...
    /* Whiteness */
    case 0xFF:
        guac_common_surface_set(current_surface, x, y, w, h, 0xFF, 0xFF, 0xFF, 0xFF);
        break;

    /* Unsupported ROP3 */
    default:
        guac_client_log(client, GUAC_LOG_INFO, "guac_rdp_gdi_dstblt(rop3=0x%x)", dstblt->bRop);
    }
    return TRUE;
}
...
BOOL guac_rdp_gdi_patblt(rdpContext *context, PATBLT_ORDER *patblt)
{
    ...
    UINT32 foreColor = patblt->foreColor;
    ...
    case 0xCC:
    case 0xF0:
        foreColor = guac_rdp_convert_color(context, patblt->foreColor);
...
BOOL guac_rdp_gdi_memblt(rdpContext *context, MEMBLT_ORDER *memblt)
{
    ...
    case 0xCC:
        ...
        if (bitmap->layer == NULL)
        {
            ...
            {
#ifdef FREERDP_2
                guac_rdp_convert_bitmap(context, memblt->bitmap);
#endif
        ...
        {
            guac_common_surface_copy(bitmap->layer->surface, x_src, y_src, w, h, current_surface, x, y);
        }
 ...
 */
static void guac_rdp_update_palette(UINT32 *guac_palette, PALETTE_UPDATE *palette)
{
    ...
    for (i = 0; i < palette->number; i++)
    {
        ...
        *guac_palette = 0xFF000000 | (entry->red << 16) | (entry->green << 8) | entry->blue;
